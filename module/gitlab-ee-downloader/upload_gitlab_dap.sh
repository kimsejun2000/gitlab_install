...
main ()
{
  ...
  gpg_keyring_path="$apt_keyrings_dir/gitlab_gitlab-ee-archive-keyring.gpg"
  gpg_key_path_old="/etc/apt/trusted.gpg.d/gitlab_gitlab-ee.gpg"
...
apt-get install --download-only -y gitlab-ee

# Copy the contents of the apt download folder
mkdir ~/gitlab
cp /var/cache/apt/archives/gitlab-ee*.deb ~/gitlab

az storage blob sync --account-name $ST_NAME -c $ST_CONTAINER --account-key $ST_KEY -s ~/gitlab
